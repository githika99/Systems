...
# Start up server.
valgrind --leak-check=full --show-leak-kinds=all --track-origins=yes -s --error-exitcode=1 --log-file=put_val_log.txt ./httpserver $port > output.txt 2>error.txt &
pid=$!
...
if [[ `wc -l < put_val_log.txt` -gt 7 ]]; then
